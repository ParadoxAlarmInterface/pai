...
def generate_property_test_parameters():
    logfile = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "properties.log"
    )
...
def test_property_map_value():
    change = Change(
        property="arm", new_value=True, old_value=None, type="partition", key="Fridge"
    )
...
@pytest.mark.parametrize(
    "property,type,value,partition,label", generate_property_test_parameters()
)
def test_property(property, type, value, partition, label):
    change = Change(
        property=property, new_value=value, old_value=None, type=type, key=label
    )
    evt = ChangeEvent(change_object=change, property_map=Panel_EVO192.property_map)
