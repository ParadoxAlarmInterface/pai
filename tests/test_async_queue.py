...
def test_handler_two_messages(mocker):
    ...
    loop.run_until_complete(asyncio.gather(task_handle_wait, task_get_eeprom, loop=loop))

    assert 1 == len(mh.handlers)

    assert task_handle_event1.result() == "event"  # failed to parse response message return None. Maybe needs to throw something.
