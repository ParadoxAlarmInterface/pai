...
def get_interface(mocker):
    ...
    mocker.MagicMock.__await__ = lambda x: async_magic().__await__() # Deal with await error
...
async def test_validate_challenge(mocker):
    ...
    try:
        ...
        assert interface._validate_command_with_challenge('arm XXXX') is None
    
        resp = calc_response(interface.challenge, SECRET, cfg.MQTT_CHALLENGE_ROUNDS)

        assert interface._validate_command_with_challenge(f'arm {resp}') == 'arm'
...
async def test_auth_output_control(mocker):
    ...
    try:
        ...
        interface.mqtt.publish.assert_called_once_with(f"{cfg.MQTT_BASE_TOPIC}/{cfg.MQTT_STATES_TOPIC}/{cfg.MQTT_CHALLENGE_TOPIC}", interface.challenge, 2, True)
        ...
        interface.mqtt.publish.assert_called_with(f"{cfg.MQTT_BASE_TOPIC}/{cfg.MQTT_STATES_TOPIC}/{cfg.MQTT_CHALLENGE_TOPIC}", interface.challenge, 2, True)
