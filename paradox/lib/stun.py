...
def read_mapped_address(attr_type, attr_body, attr_len):
    ...
    return dict(
        name=STUN_ATTRIBUTE_NAMES[attr_type], ip=ip, port=port, family=family_text
    )
...
def read_attributes(attributes, body_length):
    ...
    while pos < body_length:
        ...
        attr_len = int(
            binascii.b2a_hex(attributes[pos + 2 : pos + 4]), 16
        )  # 16bit (2bytes)
        ...
        if attr_type in (MAPPED_ADDRESS, XOR_MAPPED_ADDRESS, b"\x00\x20"):
            parsed_attributes.append(
                read_mapped_address(attr_type, attr_body, attr_len)
            )
        else:
